...
def test_crawler_pipeline_integration(monkeypatch):
    ...
    pipeline = DocumentChunkPipeline(max_tokens=1000, min_tokens=500, desired_overlap=100)
    ...
    # Check overlap: For chunks from the same source, the current chunk should start with
    ...
    for source, docs in source_groups.items():
        if len(docs) > 1:
            for i in range(1, len(docs)):
                ...
                actual_start = " ".join(default_tokenizer(docs[i]["text"])[:len(expected_overlap.split())])
