...
class DocumentChunkPipeline:
    ...
    - overlap 적용: 동일 source의 인접 청크에 대해 앞 청크의 마지막 desired_overlap(100) 토큰을 현재 청크 앞에 추가합니다.
    """

    def __init__(self, max_tokens: int = 1000, min_tokens: int = 500, desired_overlap: int = 100,
                 length_function=None, tokenizer=None):
    ...
    def merge_chunks(self, docs: List[Dict]) -> List[Dict]:
        ...
        while i < len(docs):
            ...
            if self.length_function(merged_text) >= self.min_tokens:
                ...
                j = i + 1
                while self.length_function(merged_text) < self.min_tokens and j < len(docs) and docs[j].get("source",
                                                                                                            "") == source:
...
if __name__ == "__main__":
    ...
    crawler_results = [
        ...
        {
            "source": "https://example.com",
            "text": "세 번째 문단 " * 40,  # 약 120 토큰 정도 (합병하면 60+90+120 ≒ 270 토큰, 여전히 부족할 수 있음)
            "metadata": {"source": "https://example.com"}
        },
        {
